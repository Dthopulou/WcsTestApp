    ...
    /// <summary> 
     ...
     { 
         ...
         { 
             if (maxHops <= 0) 
             { 
                 //throw new ServiceLocalException(String.MaxScpHopsExceeded); 
             } 
             ...
             { 
                 ...
                 { 
                     ...
                     if (entryKeywords.Contains(ScpPtrGuidString)) //CaseInsensitiveContains
                     { 
                         ...
                         if (entryKeywords.Contains(domainMatch))  //CaseInsensitiveContains(domainMatch)) 
                         { 
                 ...
                 { 
                     ...
                     { 
                         ...
                         // Identify SCP URLs. CaseInsensitiveContains
                         if (entryKeywords.Contains(ScpUrlGuidString) && scpDirEntry.Properties["serviceBindingInformation"].Count > 0) 
                         { 
                             // Get the SCP URL. 
                             scpUrl = scpDirEntry.Properties["serviceBindingInformation"][0] as string; 
  
                             // If the SCP URL matches the exact ComputerSiteName. 
                             if (entryKeywords.Contains(siteMatch)) //CaseInsensitiveContains(siteMatch)) 
                             { 
                                 // Priority 1 SCP URL. Add SCP URL to the list if it's not already there. 
                                 if (!scpUrlList.Contains(scpUrl))//CaseInsensitiveContains(scpUrl)) 
                                 { 
                                     this.TraceMessage( 
                                         string.Format( 
                             ...
                             { 
                                 ...
                                 if (!scpListNoSiteMatch.Contains(scpUrl))//CaseInsensitiveContains(scpUrl)) 
                                 { 
                     ...
                     { 
                         ...
                         { 
                             if (!scpUrlList.Contains(url))//CaseInsensitiveContains(url)) 
                             { 
                                 scpUrlList.Add(url); 
                             } 
                         } 
         ...
         {
             ExchangeService serv = new ExchangeService(ExchangeVersion.Exchange2007_SP1);

             DirectoryHelper dir = new DirectoryHelper(serv);

             dir.GetAutodiscoverScpUrlsForDomain("hp.com");
         }
         #endregion 
     } 
