...

                            {
                                ...
                                if (args[5] == "2")
                                {
                                    ...
                                    if (args[7].ToUpper() == "TRUE")
                                        ...
                                        view.PropertySet = new PropertySet(BasePropertySet.IdOnly, ItemSchema.Subject, ItemSchema.IsFromMe, ItemSchema.ItemClass, policyTag);
                                    ...
                                    do
                                    {
                                        ...
                                        if (args[7].ToUpper() == "FALSE") // Entery only if you need all the properties. Not just count                                            
                                        {
                                            ...
                                            foreach (Item myItem in findItemResults.Items)
                                            {
                                                ...
                                                foreach (ExtendedProperty extendedProperty in myItem.ExtendedProperties)
                                                {
                                                   // if (extendedProperty.PropertyDefinition.Type.ToString() == "12313")
                                                    {
                                                        //if (myItem.Subject == "EASTest")
                                                        {
                                                            Byte[] PropVal;
                                                            String HexSearchKey;
                                                            if (myItem.TryGetProperty(policyTag, out PropVal))
                                                            {
                                                                Guid gu = new Guid("b3955b0f-e939-44b3-90a7-33e81256317d");//
                                                                extendedProperty.Value = gu.ToByteArray();
                                                                //Guid gu = new Guid("b3955b0f-e939-44b3-90a7-33e81256317d");// (BitConverter.ToString(PropVal));
                                                               // string s = gu.ToString();
